 ...
 * Uses explicit type field if available, falls back to name-based detection
 * @param {Object} event - Event object with type and/or name property
 * @returns {boolean}
 */
function isDivingEvent(event) {
  if (!event) return false;
  if (event.type) return event.type === 'diving';
  return event.name === 'Diving';
}
...
function isRelayEvent(event) {
  if (!event) return false;
  if (event.type) return event.type === 'relay';
  return !!(event.name && event.name.includes('Relay'));
}
 ...
 * Uses explicit type field if available, falls back to name-based detection
 * @param {Object} event - Event object with type and/or name property
 * @returns {string} 'diving', 'relay', or 'individual'
 */
function getEventType(event) {
  if (event && event.type) return event.type;
  if (isDivingEvent(event)) return 'diving';
  if (isRelayEvent(event)) return 'relay';
  return 'individual';
}
